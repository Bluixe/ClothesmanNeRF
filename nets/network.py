...
from nets.canonical_mlp import CanonicalMLP
from nets.mweight_vol_decoder import MotionWeightVolumeDecoder
from nets.no_rigid_motion_mlp import NonRigidMotionMLP
from nets.pose_decoder import BodyPoseRefiner
from nets.fourier import get_fourier
from nets.hannw_fourier import get_hannw_fourier
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.mweight_vol_decoder = MotionWeightVolumeDecoder(
            embedding_size=mweight_volume_embedding_size,
            volume_size=mweight_volume_volume_size,
            total_bones=total_bones)
        
        # non-rigid motion st positional encoding
        self.get_non_rigid_embedder = get_hannw_fourier

        # non-rigid motion MLP
        _, non_rigid_pos_embed_size = self.get_non_rigid_embedder(
            non_rigid_motion_mlp_multires,
            non_rigid_motion_mlp_i_embed)
        self.non_rigid_mlp = NonRigidMotionMLP(
            pos_embed_size=non_rigid_pos_embed_size,
            condition_code_size=non_rigid_motion_mlp_condition_code_size,
            mlp_width=non_rigid_motion_mlp_mlp_width,
            mlp_depth=non_rigid_motion_mlp_mlp_depth,
            skips=non_rigid_motion_mlp_skips)
        self.non_rigid_mlp = nn.DataParallel(self.non_rigid_mlp)

        # canonical positional encoding
        get_embedder = get_fourier
        cnl_pos_embed_fn, cnl_pos_embed_size = get_embedder(
            canonical_mlp_multires, 
            canonical_mlp_i_embed)
        ...
        self.cnl_mlp = CanonicalMLP(
            ...
            mlp_depth=canonical_mlp_mlp_depth,
            mlp_width=canonical_mlp_mlp_width,
            skips=skips)
        self.cnl_mlp = nn.DataParallel(self.cnl_mlp)

        # pose decoder MLP
        self.pose_decoder = BodyPoseRefiner(
            embedding_size=pose_decoder_embedding_size,
            mlp_width=pose_decoder_mlp_width,
            mlp_depth=pose_decoder_mlp_depth
        )
    ...
    def forward(self,
        ...
        non_rigid_pos_embed_fn, _ = self.get_non_rigid_embedder(
            multires=non_rigid_motion_mlp_multires,                         
            is_identity=non_rigid_motion_mlp_i_embed,
            iter_val=iter_val)
